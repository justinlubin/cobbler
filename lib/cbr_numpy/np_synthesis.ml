...
let expand : int -> expr -> expr list =
  fun _ e ->
    ...
    | Str s -> [Str s]
    | Name id -> [Name id]
    | Hole (Number, _) -> [Call (Name "sum", [Hole (Array, Util.gensym "hole")])]
    | Hole (Array, _) -> 
      [ Call (Name "mul", [Hole (Array, Util.gensym "hole"); Hole (Array, Util.gensym "hole")])
      ; Call (Name "add", [Hole (Array, Util.gensym "hole"); Hole (Array, Util.gensym "hole")])
      ]
    | Index (head, index) -> 
