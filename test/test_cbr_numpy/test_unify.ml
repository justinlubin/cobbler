...
let candidate2 : program =
  ...
  , [ Assign
        ( Name "x"
        , Call (Name "+", [ Call (Name "*", [ Hole "1"; Hole "2" ]); Num 3 ]) )
    ] )
...
let%test_unit "more complex hole substitution" =
  [%test_result: substitutions option]
    (unify_naive reference2 candidate1)
    ~expect:
      (Some
